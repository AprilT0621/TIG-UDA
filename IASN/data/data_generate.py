...
class DataGenerator_PointNet:
    def __init__(self, df, channel="channel_first",
                 ...
                 toprint=False,
                 aug2=False,
                 data_dir='/media/aprilt/Disk2/ljp/TIG-UDA/MSCMR/'):
    ...
    def get_image_paths(self, id):
        if self._source == "source":
            if self._phase == "train":
                img_path = os.path.join(self._data_dir, 'trainA/{}.png'.format(id))
                mask_path = os.path.join(self._data_dir, 'trainAmask/{}.png'.format(id))
            else:
                img_path = os.path.join(self._data_dir, 'testA/{}.png'.format(id))
                mask_path = os.path.join(self._data_dir, 'testAmask/{}.png'.format(id))

        else:
            if self._phase == "train":
                img_path = os.path.join(self._data_dir, 'trainB/{}.png'.format(id))
                mask_path = os.path.join(self._data_dir, 'trainBmask/{}.png'.format(id))
            else:
                img_path = os.path.join(self._data_dir, 'testB/{}.png'.format(id))
                mask_path = os.path.join(self._data_dir, 'testBmask/{}.png'.format(id))
...
def get_images_masks(img_paths, mask_paths, crop_size=224):
    imgs, masks = [], []
...
if __name__ == "__main__":
    ids_train = ImageProcessor.split_data("/TIG-UDA/MSCMRSeg/trainA.csv")
    ids_valid = ImageProcessor.split_data("/TIG-UDA/MSCMRSeg/testA.csv")
    ids_train_lge = ImageProcessor.split_data("/TIG-UDA/MSCMRSeg/trainB.csv")
    ids_valid_lge = ImageProcessor.split_data("/TIG-UDA/MSCMRSeg/testB.csv")
    source_train_generator = DataGenerator_PointNet(df=ids_train, batch_size=16, phase="train", apply_online_aug=True, source="source", crop_size=224, n_samples=64)
