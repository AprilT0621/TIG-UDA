import torch
import itertools
from stage1.util.image_pool import ImagePool
from .base_model import BaseModel
from . import networks


class ISNModel(BaseModel):
    """
    This class implements the Image Synthesis Network in the paper, for learning image-to-image translation matched data.
    """
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'D_B', 'G_B', 'cycle_B', 'idt_B','SEG_A','SEG_B','fusiongan','ssm']
        # specify the images you want to save/display. The training/test scripts will call <BaseModel.get_current_visuals>
        visual_names_A = ['real_A', 'fake_B', 'rec_A']
        visual_names_B = ['real_B', 'fake_A', 'rec_B']
        visual_names_S = ['mask_vis','CTpredict_A_vis','CTpredict_B_vis']

        if self.isTrain and self.opt.lambda_identity > 0.0:  # if identity loss is used, we also visualize idt_B=G_A(B) ad idt_A=G_A(B)
            visual_names_A.append('idt_B')
            visual_names_B.append('idt_A')

        # self.visual_names = visual_names_A + visual_names_B + visual_names_S  # combine visualizations for A and B
        self.visual_names = visual_names_A + visual_names_B  # combine visualizations for A and B  加的
        # specify the models you want to save to the disk. The training/test scripts will call <BaseModel.save_networks> and <BaseModel.load_networks>.
        if self.isTrain:
            self.model_names = ['Encoder_A', 'Encoder_B','Decoder_A','Decoder_B', 'Fusion','D_A', 'D_B','FCN_A','FCN_B']
        else:  # during test time, only load Gs
            self.model_names = ['G_A', 'G_B']
            # self.model_names = ['Encoder_A', 'Encoder_B', 'Decoder_A', 'Decoder_B', 'Fusion']
    ...
    def forward(self):
        ...
        self.ft1 = self.pw1(self.fake_B)
        self.ft2 = self.pw2(self.CTpredict_B)
        # if self.isTrain:
    ...
    def backward_FCN_A(self):
        """Calculate segmentation loss for fake CT iamges"""
    ...
    def backward_FCN_B(self):
        """Calculate segmentation loss for fake fake MR iamges"""
        for param in self.netFCN_B.parameters():
            param.requires_grad = True
        CTpredict, feature_hct = self.netFCN_B(self.rec_A.detach())
        ct_predict=torch.sigmoid(CTpredict)
        loss_ct=self.criterionSEG(ct_predict.cuda(), self.mask.cuda())
        ft1 = self.pw1(self.fake_B.detach())
        ft2 = self.pw2(CTpredict)
        self.loss_ssm = networks.SSloss(ft1, ft2)
        loss_add = loss_ct + 0.1 * self.loss_ssm
        #(loss_ct).backward()
        loss_add.backward()
    ...
    def backward_Funsion(self):
        ...
        gan_loss = self.criterionGAN(B_out, True)
    ...
    def backward_D_domain(self):
        ...
        d_loss1 = self.criterionGAN(A_out, True)
        d_loss1.backward()
        # d_loss2 = self.criterionGAN(B_out, Blabel)
        d_loss2 = self.criterionGAN(B_out, False)
        d_loss2.backward()
    ...
    def backward_G(self):
        ...
        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + 5*self.loss_cycle_A + 5*self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B+50*self.loss_SEG_A+50*self.loss_SEG_B
    ...
    def optimize_parameters(self):
        ...
        # G_A and G_B
        self.set_requires_grad([self.netD_A, self.netD_B,self.netFCN_A], False)  # Ds require no gradients when optimizing Gs
        for param in self.netFCN_A.parameters():
            param.requires_grad = False
        ...
        for param in self.netFCN_B.parameters():
            param.requires_grad = True
        self.optimizer_FCN.zero_grad()
        self.backward_FCN_A()
        self.backward_FCN_B()
        self.optimizer_FCN.step()